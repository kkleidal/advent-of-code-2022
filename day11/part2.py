...
def _parse_expr(expr: str) -> Expr:
    if " " in expr:
        ...
        return {
            ...
            "-": Sub,
            "*": Mult,
            #  "/": Div, Not 100% sure how this affects the modular base
        }[tokens[1]](
...
def parse():
    ...
    return monkeys, math.lcm(*bases)

def simulate_round(monkeys, mod_base, items_by_monkey=None, verbose=False, inspect_cb=None):
...
if __name__ == "__main__":
    ...
    monkeys, mod_base = parse()
    total_inspects = [0 for _ in monkeys]
    def inspect_cb(mi, x):
        total_inspects[mi] += 1
    items_by_monkey = None
    for rnd in range(10000):
        items_by_monkey = simulate_round(monkeys, mod_base, items_by_monkey, inspect_cb=inspect_cb)
